...
class TrainingData {
  // The full list is split across 5 helper methods to keep each method
  // within the Dart VM unoptimised-JIT bytecode limit (avoids startup crash).
  static List<Technique>? _cache;
  static List<Technique> get techniques => _cache ??= [
        ..._group1(),
        ..._group2(),
        ..._group3(),
        ..._group4(),
...
def shortestPath(grid):
  ...
  static List<Technique> _group3() => [
    ...
    return dp[m][n]''',
          ...
          spaceComplexity:
              'O(m * n); often reducible to O(min(m, n)) with a rolling array',
          tips: [
        ...
        const Technique(
          ...
          steps: [
            TechniqueStep(
              ...
              description:
                  'Each node holds a dict/array of children (one per character) and a boolean `is_end` flag.',
            ),
            TechniqueStep(
              stepNumber: 2,
              title: 'Insert a word',
              description:
                  'Iterate through each character. If no child exists for it, create one. After the last character, mark `is_end = True`.',
            ),
            TechniqueStep(
              stepNumber: 3,
              title: 'Search or startsWith',
              description:
                  'Walk down the Trie following each character. If any character is missing, return False. For `search`, also check `is_end`.',
            ),
          ],
          codeExamples: [
            CodeExample(
              ...
              language: 'python',
              explanation:
                  'Classic Trie implementation with insert, search, and prefix check in O(L) time.',
              code: '''class TrieNode:
    def __init__(self):
        self.children = {}
        self.is_end = False
    ...
    def startsWith(self, prefix: str) -> bool:
        ...
        return True''',
            ...
            CodeExample(
              ...
              language: 'python',
              explanation:
                  'Find all words from a dictionary that exist in a 2D board. Build a Trie then DFS the board.',
        ...
        const Technique(
          ...
          steps: [
            TechniqueStep(
              ...
              title: 'Initialize',
              description:
                  'Create parent[] and rank[] arrays. Each element is its own parent (parent[i] = i), rank = 0.',
            ),
            TechniqueStep(
              stepNumber: 2,
              title: 'Find with path compression',
              description:
                  'Recursively find the root. On the way back up, set parent[x] = root directly (path compression).',
            ),
            TechniqueStep(
              stepNumber: 3,
              title: 'Union by rank',
              description:
                  'Find both roots. If different, attach the lower-rank tree under the higher-rank one. Increment rank if they were equal.',
            ),
          ],
          codeExamples: [
            CodeExample(
              ...
              language: 'python',
              explanation:
                  'Full DSU implementation. find() is nearly O(1) amortized thanks to path compression.',
    ...
    def connected(self, x, y):
        return self.find(x) == self.find(y)''',
            ...
            CodeExample(
              ...
              language: 'python',
              explanation:
                  'Union all edges, then count distinct roots. Classic Union-Find application.',
        ...
        const Technique(
          ...
          steps: [
            TechniqueStep(
              ...
              description:
                  'Decreasing stack → find next greater element. Increasing stack → find next smaller element.',
            ),
            TechniqueStep(
              stepNumber: 2,
              title: 'Iterate and pop',
              description:
                  'For each element, while the stack is not empty AND the top violates the monotonic property, pop and process. The current element is the "answer" for the popped element.',
            ),
            TechniqueStep(
              stepNumber: 3,
              title: 'Push and continue',
              description:
                  'Push the current element (or its index) onto the stack. Anything remaining in the stack at the end has no valid answer (use -1 or 0).',
            ),
          ],
          codeExamples: [
            CodeExample(
              ...
              language: 'python',
              explanation:
                  'For each element, find the next element to the right that is strictly greater. Uses a monotonic decreasing stack of indices.',
              code: '''def nextGreaterElement(nums):
    n = len(nums)
    result = [-1] * n
...
# result = [4, 2, 4, -1, -1]''',
            ...
            CodeExample(
              ...
              language: 'python',
              explanation:
                  'Classic monotonic stack problem. Maintain increasing stack of bar indices. Pop when a shorter bar is found and compute max area.',
