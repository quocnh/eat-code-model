    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "init":
            if let arguments = call.arguments as? [String: Any],
               ...
               let temperature = (arguments["temperature"] as? NSNumber).map({ Float($0.doubleValue) }),
               ...
               let topK = arguments["topK"] as? Int {
                do {
                    ...
                    result(FlutterError(code: "ERROR", message: "Failed to initialize gemma: \(error.localizedDescription)", details: nil))
                }
            } else {
